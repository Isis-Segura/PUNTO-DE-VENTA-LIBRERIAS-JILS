            ...
            <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(15,23,42,.08);">
                <tr>
                    <td style="background:linear-gradient(135deg,#1e3a8a,#3b82f6);padding:20px 28px;color:#fff;">
                        ...
                        <div style="font-size:22px;font-weight:700;margin-top:4px;">{{ __('Tu cuenta ha sido creada') }}</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:28px;">
                        <p style="margin:0 0 12px;font-size:15px;">{{ __('Hola') }} <strong>{{ $usuario->name }}</strong>,</p>
                        <p style="margin:0 0 18px;font-size:15px;line-height:1.5;color:#334155;">
                            {{ __('Un administrador te registró en el sistema de punto de venta') }}
                            <strong>{{ __('PDV JILS') }} / {{ __('Librería JILS') }}</strong>. {{ __('Estos son tus datos de acceso:') }}
                        </p>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:18px;">
                            <tr>
                                <td style="padding:14px 16px;border-bottom:1px solid #e2e8f0;">
                                    <div style="font-size:12px;color:#64748b;text-transform:uppercase;">{{ __('Usuario / correo') }}</div>
                            ...
                            <tr>
                                <td style="padding:14px 16px;">
                                    <div style="font-size:12px;color:#64748b;text-transform:uppercase;">{{ __('Contraseña temporal') }}</div>
                        ...
                        <p style="margin:0 0 8px;font-size:14px;color:#475569;line-height:1.5;">
                            {{ __('Te recomendamos iniciar sesión y cambiar la contraseña desde tu menú de usuario') }}
                            (<em>{{ __('Cambiar contraseña') }}</em>).
                        </p>
                        <p style="margin:0;font-size:13px;color:#94a3b8;">
                            {{ __('Si no esperabas este mensaje, ignóralo o contacta a la administración de la librería.') }}
                        </p>
                    </td>
                </tr>
                <tr>
                    <td style="padding:14px 28px;background:#f8fafc;border-top:1px solid #e2e8f0;font-size:12px;color:#94a3b8;text-align:center;">
                        © {{ date('Y') }} {{ __('Librería JILS') }} · {{ __('PDV JILS') }}
                    </td>
                </tr>
            </table>
        </td>
